...
requestRouter.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let emailRes = null;
      try {
        emailRes = await sendEmail.run("A new friend request from" + req.user.firstName,
          req.user.firstName + " is " + status + " in " + to
        );
        console.log("emailResult:", emailRes);
      } catch (emailErr) {
        console.log("Email failed but request saved:", emailErr.message);
      }
      ...
      res.status(400).send("ERROR: kya hogya " + err.message);
    }
  },
);
